...
import { extractSidebarItems, type SidebarItem } from './extractSidebarItems';
...
// Helper to create a skill ToolPart (completed)
function createSkillToolPart(id: string, messageId: string, skillName: string): ToolPart {
  return {
    id,
    sessionID: 'ses_test',
    messageID: messageId,
...
// Helper to create a non-task/non-skill ToolPart
function createOtherToolPart(id: string, messageId: string, toolName: string): ToolPart {
  return {
    id,
    sessionID: 'ses_test',
    messageID: messageId,
...
describe('isTaskToolPart', () => {
  ...
  it('returns false for ToolPart with other tool names', () => {
    const bashPart = createOtherToolPart('p1', 'msg_1', 'bash');
...
describe('extractSidebarItems', () => {
  describe('tasks and subtasks', () => {
    it('extracts SubtaskPart as subtask items', () => {
      ...
      const items = extractSidebarItems(group);

      expect(items).toHaveLength(1);
      expect(items[0]).toEqual<SidebarItem>({
        id: 'p1',
        label: 'explore',
        messageId: 'msg_1',
        itemType: 'subtask',
    ...
    it('extracts task ToolPart as task items', () => {
    ...
    it('ignores non-task, non-skill ToolParts', () => {
      const bashTool = createOtherToolPart('p1', 'msg_1', 'bash');
      const readTool = createOtherToolPart('p2', 'msg_1', 'read');
    ...
    it('extracts both SubtaskPart and task ToolPart from the same message', () => {
      ...
      expect(items[0].label).toBe('explore');
      expect(items[0].itemType).toBe('subtask');
      expect(items[1].id).toBe('p2');
      expect(items[1].label).toBe('general');
      expect(items[1].itemType).toBe('task');
    });

    it('extracts items from multiple assistant messages', () => {
      ...
      expect(items).toHaveLength(2);
      expect(items[0].messageId).toBe('msg_1');
      expect(items[1].messageId).toBe('msg_2');
    });
  ...
  describe('skills', () => {
    it('extracts skill ToolPart as skill items', () => {
      const skillTool = createSkillToolPart('p1', 'msg_1', 'code-review');
      const assistantMsg = createAssistantMessage('msg_1', [skillTool]);
      const group = createMessageGroup([assistantMsg]);
    ...
    it('extracts skills alongside tasks in document order', () => {
      const skillTool = createSkillToolPart('p1', 'msg_1', 'ports-and-adapters');
      const taskTool = createTaskToolPart('p2', 'msg_1', 'kotlin-pro');
      const skillTool2 = createSkillToolPart('p3', 'msg_1', 'git-commit');
      const assistantMsg = createAssistantMessage('msg_1', [skillTool, taskTool, skillTool2]);
      const group = createMessageGroup([assistantMsg]);
      ...
      expect(items[0].label).toBe('ports-and-adapters');
      expect(items[1].id).toBe('p2');
      expect(items[1].itemType).toBe('task');
      expect(items[1].label).toBe('kotlin-pro');
      expect(items[2].id).toBe('p3');
      expect(items[2].itemType).toBe('skill');
      expect(items[2].label).toBe('git-commit');
    });

    it('handles skill ToolPart with missing name in input', () => {
      const skillToolNoName: ToolPart = {
        id: 'p1',
        sessionID: 'ses_test',
        messageID: 'msg_1',
        type: 'tool',
      ...
      const assistantMsg = createAssistantMessage('msg_1', [skillToolNoName]);
      const group = createMessageGroup([assistantMsg]);

      const items = extractSidebarItems(group);

      expect(items).toHaveLength(1);
      expect(items[0].label).toBe('skill'); // Falls back to 'skill'
      expect(items[0].itemType).toBe('skill');
    });

    it('handles skill ToolPart with null input', () => {
      const skillToolNullInput: ToolPart = {
        id: 'p1',
        sessionID: 'ses_test',
        messageID: 'msg_1',
        type: 'tool',
      ...
      const assistantMsg = createAssistantMessage('msg_1', [skillToolNullInput]);
      const group = createMessageGroup([assistantMsg]);
    ...
    it('handles skill ToolPart with pending status', () => {
      const pendingSkillTool: ToolPart = {
        id: 'p1',
        sessionID: 'ses_test',
        messageID: 'msg_1',
        type: 'tool',
      ...
      const assistantMsg = createAssistantMessage('msg_1', [pendingSkillTool]);
      const group = createMessageGroup([assistantMsg]);

      const items = extractSidebarItems(group);

      expect(items).toHaveLength(1);
      expect(items[0].label).toBe('verification-before-completion');
      expect(items[0].itemType).toBe('skill');
    });
  });
  ...
  describe('task tool edge cases', () => {
    it('handles task ToolPart with pending status and empty input object', () => {
      ...
      expect(items).toHaveLength(1);
      expect(items[0].label).toBe('task'); // Falls back to 'task' when subagent_type not in input
    });
    ...
    it('handles task ToolPart with running status', () => {
      ...
      expect(items).toHaveLength(1);
      expect(items[0].label).toBe('kotlin-pro');
    });
    ...
    it('handles task ToolPart with error status', () => {
      ...
      expect(items).toHaveLength(1);
      expect(items[0].label).toBe('test-automator');
    });
    ...
    it('handles task ToolPart with missing subagent_type in input', () => {
      ...
      expect(items).toHaveLength(1);
      expect(items[0].label).toBe('task'); // Falls back to 'task'
    });
    ...
    it('uses assistant message ID for messageId, not part.messageID', () => {
      ...
      expect(items[0].messageId).toBe('msg_assistant');
    });
